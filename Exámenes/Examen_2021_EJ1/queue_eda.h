...
public:
   ...
   void voidcuela(T const& a, T const& b) {
       //Ponemos b inmediatamente despues de a

       Nodo* it = prim;
       Nodo* _a;
       Nodo* ant = prim;

       while (it->elem != a && it!=ult) it = it->sig;  //Buscamos A
       if (it->sig == nullptr) return;   //El elemento no está en la cola
       else _a = it;

       while (it->elem != b && it->sig != nullptr) 
       {
           ant = it;
           it = it->sig; //Buscamos B tras A
       }
       if (it->elem == b) {
           ant->sig = it->sig;  //El que señalaba a b ahora debe señalar al siguiente de b
           it->sig = _a->sig;
           _a->sig = it;
       }

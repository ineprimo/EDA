...
public:
   ...
   void colar(queue<T> colegas, T pringao)
   {
       // buscamos el pringao en la cola
       Nodo* actual = prim;
       while (actual != nullptr && actual->elem != pringao)
       {
           actual = actual->sig;
       }

       // pringao no esta: no hacemos nada
       if (actual == nullptr) return;

       Nodo* sigOriginal = actual->sig; // nos lo guardamos antes de modificarlo

       // tenemos actual en el pringao
       actual->sig = colegas.prim; // el siguiente sera el primero de los colegas
       colegas.ult->sig = sigOriginal; // el ultimo de los colegas ahora apunta al siguiente original del pringao.

       // actualizamos el tamaño
       nelems += colegas.size();

       // vaciamos colegas porque hemos robado sus nodos
       colegas.nelems = 0;

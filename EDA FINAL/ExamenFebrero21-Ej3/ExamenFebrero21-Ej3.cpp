...
void resolver(vector<int>& sol, int n, int k, int& maxBeneficio, int& beneficioActual,
    const vector<vector<bool>> consentimientos, const vector<vector<int>> beneficios) {
    for (int i = 0; i < n; i++)    {
        sol[k] = i;
        if (esValido(sol, n, k, consentimientos)) {
            ...
            else if (k < n - 1) resolver(sol, n, k + 1, maxBeneficio, beneficioActual, consentimientos, beneficios);
        }
        sol.pop_back();
    }
}
...
void resuelveCaso() {
    ...
    resolver(sol, n, 0, maxBeneficio, beneficioActual, consentimientos, beneficios);

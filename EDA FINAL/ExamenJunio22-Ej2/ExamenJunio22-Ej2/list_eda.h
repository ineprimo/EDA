...
public:
   ...
   void adelantaPosiciones(int const pos, int lon, int const k) {
       ...
       Nodo* primeroLista = this->fantasma->sig;
       Nodo* primeroSegmento = primeroLista;
       for (int i = 0; i < pos; ++i) primeroSegmento = primeroSegmento->sig;

       Nodo* ultimoSegmento = primeroSegmento;
       for (int i = 1; i < lon; ++i) ultimoSegmento = ultimoSegmento->sig;

       Nodo* preSegmento = primeroSegmento->ant;
       Nodo* postSegmento = ultimoSegmento->sig;

       preSegmento->sig = postSegmento;
       postSegmento->ant = preSegmento;

       Nodo* destino = primeroLista;
       for (int i = 0; i < newPos; ++i) destino = destino->sig;
       Nodo* preDestino = destino->ant;

       preDestino->sig = primeroSegmento;
       primeroSegmento->ant = preDestino;
       ultimoSegmento->sig = destino;
       destino->ant = ultimoSegmento;
   }
};

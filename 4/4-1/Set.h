...
public:
    ...
    // Complejidad: lineal O(n) siendo n el tamaño de A para la diferencia A-B
    Set<T> operator-(const Set<T>& other) const 
    {
        ...
        {
            if (array[i] == other.array[j]) // si son iguales buscamos en el siguiente indice de ambos
            {
                i++;
                j++;
            }
            else if (array[i] < other.array[j]) // sigues buscando en other mientras sea mayor
            {
                aux.array[k] = array[i];
                i++;
                k++;

...
public:
   ...
   void cuela(const T& a, const T& b) 
   {
       ...
       //El objetivo es buscar la B, pero aquí dentro encontramos la A también (busca hasta el final)
       while (actual != ult && !foundB)
       {
           //Si no se ha encontrado ya una a, es esa
           if (actual->elem == a && !foundA) {
               posA = actual;
               foundA = true;
           }

           // Una vez que ya hemos encontrado la A, buscamos la B
           // Si el siguiente elemento coincide, hacemos cosas
           if (foundA && actual->sig->elem == b)
           {
               posB = actual->sig;
               actual->sig = posB->sig;

...
/*
    ...
    void partition(int pivote)
    {
        bool foundPos = false;
        // Punteros
        Nodo* actual = this->begin;
        Nodo* peques = this->begin;

        //Buscamos la posición del pivote (menor o igual, ya que desde ahí en adelante empezaremos a modificar)
        while (actual != this->end && !foundPos)
        {
            //Si el siguiente es mayor y el anterior es el pivote o uno menor, lo habremos encontrado 
            if (actual->sig->elem == pivote+1 && actual->elem <= pivote)
            {
                foundPos = true;
            }
            ...
            {
                actual->sig->elem > pivote&& actual->elem <= pivote;
            }

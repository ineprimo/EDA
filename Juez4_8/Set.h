...
public:
	...
	// Complejidad O(n), porque solo recorremos una vez el array, según el tamaño de este, que es n
	Set<T> operator-(const Set<T>& other) const {
		...
		while (i < nelems && j < other.nelems) {
			...
			//Elemento en ambos - avanzamos los dos vectores
			if (array[i] == other.array[j]) {
				i++;
				j++;
			}
			//Añadimos al array de salida
		...
		//Si sobran elementos en el array
		if (i < nelems) {
			while (i < nelems) {
				a.array[k] = array[i];
				k++;

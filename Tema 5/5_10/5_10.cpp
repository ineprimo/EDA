...
public:
    ...
    //Complejidad O(n + m) Siendo n el numero de elementos del vector y m 
    // el sumatorio de las multiplicaciones que hay que hacer
    void replicaElems(vector<int> const& mult) {
        
        Nodo* ite = this->prim;
        Nodo* pre_ite = nullptr;
        Nodo* siguiente = ite->sig;
        Nodo* aux = nullptr;
        ...
        for (int m : mult) {
            ...
            if (m == 0) {
                ...
                if (this->ult == ite) // si es el ultimo
                    this->ult = pre_ite;
